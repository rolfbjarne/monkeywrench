...
WHERE Revision.lane_id = @lane_id AND ";
		...
		public GetTestResultsResponse GetTestResults (WebServiceLogin login, string [] revisions, string filename)
		{
			...
			using (DB db = new DB ()) {
				...
				using (IDbCommand cmd = db.CreateCommand ()) {
					cmd.CommandText = @"
SELECT 0 as id, workfile.id as workfile_id, file.md5, workfile.filename, work.command_id, revision.revision, revisionwork.id as revisionwork_id, revisionwork.lane_id, revisionwork.host_id, revisionwork.revision_id 
